...
$url1 = Get-FilenameFromRegex 'http://www.filehippo.com/download_shockwave/' '(?smi)download_shockwave/(\d+)?/?"[^>]+>.?Shockwave Player {{PackageVersion}}' 'http://www.filehippo.com/download_shockwave/$1/'
if (! $url1) {
  ...
  while (! $url1 -and $page -le $lastPage) {
    $url1 = Get-FilenameFromRegex "http://www.filehippo.com/download_shockwave/history/$page/" '(?smi)download_shockwave/(\d+)?/?"[^>]+>.?Shockwave Player {{PackageVersion}}' 'http://www.filehippo.com/download_shockwave/$1/'
    $page++
  }
  if (! $url1) {
