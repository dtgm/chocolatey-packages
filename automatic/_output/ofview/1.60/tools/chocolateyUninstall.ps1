...
if (Test-Path -PathType Container (Join-Path $chocoLib 'ofview.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package ofview, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\ofview.*).Name
  foreach ($i in $list) {
