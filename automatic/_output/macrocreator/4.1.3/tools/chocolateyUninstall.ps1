...
if (Test-Path -PathType Container (Join-Path $chocoLib 'macrocreator.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package macrocreator, please also run the command:"
  $text += " `n`tchoco uninstall "
  $list = (Get-ChildItemDir $chocoLib\macrocreator.*).Name
  foreach ($i in $list) {
    $text += "$i "
  }
  $text += "-y`n"
