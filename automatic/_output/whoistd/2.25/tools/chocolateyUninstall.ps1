...
if (Test-Path -PathType Container (Join-Path $chocoLib 'whoistd.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package whoistd, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\whoistd.*).Name
  foreach ($i in $list) {
