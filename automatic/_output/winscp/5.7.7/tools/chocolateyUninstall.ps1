...
if (Test-Path -PathType Container (Join-Path $chocoLib 'winscp.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package winscp, please also run the command:"
  $text += " `n`tchoco uninstall "
  $list = (Get-ChildItemDir $chocoLib\winscp.*).Name
  foreach ($i in $list) {
    $text += "$i "
  }
  $text += "-y`n"
