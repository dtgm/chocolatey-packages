...
if (Test-Path -PathType Container (Join-Path $chocoLib 'regscanner.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package regscanner, please also run the command:"
  Write-Host " `n`tchoco uninstall " -NoNewLine
  $list = (Get-ChildItemDir $chocoLib\regscanner.*).Name
  foreach ($i in $list) {
