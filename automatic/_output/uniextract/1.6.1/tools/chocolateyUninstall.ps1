...
if (Test-Path -PathType Container (Join-Path $chocoLib 'uniextract.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package uniextract, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\uniextract.*).Name
  foreach ($i in $list) {
