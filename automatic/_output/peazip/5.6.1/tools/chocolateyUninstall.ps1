...
if (Test-Path -PathType Container (Join-Path $chocoLib 'peazip.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package peazip, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\peazip.*).Name
  foreach ($i in $list) {
