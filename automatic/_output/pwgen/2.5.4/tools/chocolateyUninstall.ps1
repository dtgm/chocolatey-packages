...
if (Test-Path -PathType Container (Join-Path $chocoLib 'pwgen.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package pwgen, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\pwgen.*).Name
  foreach ($i in $list) {
