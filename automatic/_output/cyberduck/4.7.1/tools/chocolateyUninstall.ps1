...
if (Test-Path -PathType Container (Join-Path $chocoLib 'cyberduck.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package cyberduck, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\cyberduck.*).Name
  foreach ($i in $list) {
