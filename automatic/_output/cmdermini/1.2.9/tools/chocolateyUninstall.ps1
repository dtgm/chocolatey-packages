...
if (Test-Path -PathType Container (Join-Path $chocoLib 'cmdermini.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package cmdermini, please also run the command:"
  Write-Host " `n`tchoco uninstall " -NoNewLine
  $list = (Get-ChildItemDir $chocoLib\cmdermini.*).Name
  foreach ($i in $list) {
