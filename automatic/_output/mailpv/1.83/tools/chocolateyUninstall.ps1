...
if (Test-Path -PathType Container (Join-Path $chocoLib 'mailpv.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package mailpv, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\mailpv.*).Name
  foreach ($i in $list) {
