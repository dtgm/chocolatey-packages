...
if (Test-Path -PathType Container (Join-Path $chocoLib 'manictime.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package manictime, please also run the command:"
  $text += " `n`tchoco uninstall "
  $list = (Get-ChildItemDir $chocoLib\manictime.*).Name
  foreach ($i in $list) {
    $text += "$i "
  }
  $text += "-y`n"
