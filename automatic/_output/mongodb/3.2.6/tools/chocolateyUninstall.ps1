...
if (Test-Path -PathType Container (Join-Path $chocoLib 'mongodb.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package mongodb, please also run the command:"
  $text += " `n`tchoco uninstall "
  $list = (Get-ChildItemDir $chocoLib\mongodb.*).Name
  foreach ($i in $list) {
    $text += "$i "
  }
  $text += "-y`n"
