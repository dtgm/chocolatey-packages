...
if (Test-Path -PathType Container (Join-Path $chocoLib 'handbrake.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package handbrake, please also run the command:"
  $text += " `n`tchoco uninstall "
  $list = (Get-ChildItemDir $chocoLib\handbrake.*).Name
  foreach ($i in $list) {
    $text += "$i "
  }
  $text += "-y`n"
