...
if (Test-Path -PathType Container (Join-Path $chocoLib 'rufus.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package rufus, please also run the command:"
  $text += " `n`tchoco uninstall "
  $list = (Get-ChildItemDir $chocoLib\rufus.*).Name
  foreach ($i in $list) {
    $text += "$i "
  }
  $text += "-y`n"
