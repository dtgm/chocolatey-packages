...
$checksum = 'e07fe9fc8b62ee0ccecfe50b02f0b92f7490970b'
$checksumType = 'sha1'
...
$url1 = Get-FilenameFromRegex 'http://www.filehippo.com/download_adwcleaner/' '(?smi)download_adwcleaner/(\d+)?/?"[^>]+>.?AdwCleaner 5.028' 'http://www.filehippo.com/download_adwcleaner/$1/'
if (! $url1) {
  ...
  while (! $url1 -and $page -le $lastPage) {
    $url1 = Get-FilenameFromRegex "http://www.filehippo.com/download_adwcleaner/history/$page/" '(?smi)download_adwcleaner/(\d+)?/?"[^>]+>.?AdwCleaner 5.028' 'http://www.filehippo.com/download_adwcleaner/$1/'
    $page++
  }
  if (! $url1) {
