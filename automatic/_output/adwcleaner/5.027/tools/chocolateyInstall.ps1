...
$checksum = 'c2fd1bba80113230fa4d21951d22b4682639d585'
$checksumType = 'sha1'
...
$url1 = Get-FilenameFromRegex 'http://www.filehippo.com/download_adwcleaner/' '(?smi)download_adwcleaner/(\d+)?/?"[^>]+>.?AdwCleaner 5.027' 'http://www.filehippo.com/download_adwcleaner/$1/'
if (! $url1) {
  ...
  while (! $url1 -and $page -le $lastPage) {
    $url1 = Get-FilenameFromRegex "http://www.filehippo.com/download_adwcleaner/history/$page/" '(?smi)download_adwcleaner/(\d+)?/?"[^>]+>.?AdwCleaner 5.027' 'http://www.filehippo.com/download_adwcleaner/$1/'
    $page++
  }
  if (! $url1) {
