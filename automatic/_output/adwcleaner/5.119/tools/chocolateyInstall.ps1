...
$checksum = '871ea7ce37bb4a9b3c600640a9865de0cf0cca153f1cc2cfbe073b3e2a47d3d2'
$checksumType = 'sha256'
...
$url1 = Get-FilenameFromRegex 'http://www.filehippo.com/download_adwcleaner/' '(?smi)download_adwcleaner/(\d+)?/?"[^>]+>.?AdwCleaner 5.119' 'http://www.filehippo.com/download_adwcleaner/$1/'
if (! $url1) {
  ...
  while (! $url1 -and $page -le $lastPage) {
    $url1 = Get-FilenameFromRegex "http://www.filehippo.com/download_adwcleaner/history/$page/" '(?smi)download_adwcleaner/(\d+)?/?"[^>]+>.?AdwCleaner 5.119' 'http://www.filehippo.com/download_adwcleaner/$1/'
    $page++
  }
  if (! $url1) {
