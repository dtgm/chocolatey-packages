...
$checksum = '747c5223ceb32c417933b420aeb715874f47136a'
$checksumType = 'sha1'
...
$url1 = Get-FilenameFromRegex 'http://www.filehippo.com/download_adwcleaner/' '(?smi)download_adwcleaner/(\d+)?/?"[^>]+>.?AdwCleaner 5.109' 'http://www.filehippo.com/download_adwcleaner/$1/'
if (! $url1) {
  ...
  while (! $url1 -and $page -le $lastPage) {
    $url1 = Get-FilenameFromRegex "http://www.filehippo.com/download_adwcleaner/history/$page/" '(?smi)download_adwcleaner/(\d+)?/?"[^>]+>.?AdwCleaner 5.109' 'http://www.filehippo.com/download_adwcleaner/$1/'
    $page++
  }
  if (! $url1) {
