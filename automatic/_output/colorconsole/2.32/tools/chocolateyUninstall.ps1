...
if (Test-Path -PathType Container (Join-Path $chocoLib 'colorconsole.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package colorconsole, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\colorconsole.*).Name
  foreach ($i in $list) {
