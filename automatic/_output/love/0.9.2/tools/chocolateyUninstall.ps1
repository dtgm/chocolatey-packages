...
if (Test-Path -PathType Container (Join-Path $chocoLib 'love.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package love, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\love.*).Name
  foreach ($i in $list) {
