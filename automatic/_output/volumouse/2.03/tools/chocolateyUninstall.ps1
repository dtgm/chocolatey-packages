...
if (Test-Path -PathType Container (Join-Path $chocoLib 'volumouse.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package volumouse, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\volumouse.*).Name
  foreach ($i in $list) {
