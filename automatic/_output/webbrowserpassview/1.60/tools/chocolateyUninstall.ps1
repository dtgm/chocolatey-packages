...
if (Test-Path -PathType Container (Join-Path $chocoLib 'webbrowserpassview.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package webbrowserpassview, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\webbrowserpassview.*).Name
  foreach ($i in $list) {
