...
if (Test-Path -PathType Container (Join-Path $chocoLib 'processhacker.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package processhacker, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\processhacker.*).Name
  foreach ($i in $list) {
