...
if (Test-Path -PathType Container (Join-Path $chocoLib 'fastresolver.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package fastresolver, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\fastresolver.*).Name
  foreach ($i in $list) {
