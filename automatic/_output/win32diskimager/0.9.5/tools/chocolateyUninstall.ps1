...
if (Test-Path -PathType Container (Join-Path $chocoLib 'win32diskimager.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package win32diskimager, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\win32diskimager.*).Name
  foreach ($i in $list) {
