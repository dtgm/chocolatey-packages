...
if (Test-Path -PathType Container (Join-Path $chocoLib 'sysexp.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package sysexp, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\sysexp.*).Name
  foreach ($i in $list) {
