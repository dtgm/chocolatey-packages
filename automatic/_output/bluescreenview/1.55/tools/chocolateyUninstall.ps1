...
if (Test-Path -PathType Container (Join-Path $chocoLib 'bluescreenview.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package bluescreenview, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\bluescreenview.*).Name
  foreach ($i in $list) {
