...
if (Test-Path -PathType Container (Join-Path $chocoLib 'bulletspassview.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package bulletspassview, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\bulletspassview.*).Name
  foreach ($i in $list) {
