...
$installerType = 'exe'
$checksum = 'e1915de2ff18022d5b15fa05c570c8852389dee4'
$checksumType = 'sha1'
$silentArgs = '/S /NCRC'
$validExitCodes = @(0)
...
$url1 = Get-FilenameFromRegex 'http://www.filehippo.com/download_shockwave/' '(?smi)download_shockwave/(\d+)?/?"[^>]+>.?Shockwave Player 12.2.2.172' 'http://www.filehippo.com/download_shockwave/$1/'
if (! $url1) {
  ...
  while (! $url1 -and $page -le $lastPage) {
    $url1 = Get-FilenameFromRegex "http://www.filehippo.com/download_shockwave/history/$page/" '(?smi)download_shockwave/(\d+)?/?"[^>]+>.?Shockwave Player 12.2.2.172' 'http://www.filehippo.com/download_shockwave/$1/'
    $page++
  }
  if (! $url1) {
