...
$installerType = 'exe'
$checksum = '6fbb68c1b127a81e8be818fc020cb9266acd3c3c'
$checksumType = 'sha1'
$silentArgs = '/S /NCRC'
$validExitCodes = @(0)
...
$url1 = Get-FilenameFromRegex 'http://www.filehippo.com/download_shockwave/' '(?smi)download_shockwave/(\d+)?/?"[^>]+>.?Shockwave Player 12.2.3.183' 'http://www.filehippo.com/download_shockwave/$1/'
if (! $url1) {
  ...
  while (! $url1 -and $page -le $lastPage) {
    $url1 = Get-FilenameFromRegex "http://www.filehippo.com/download_shockwave/history/$page/" '(?smi)download_shockwave/(\d+)?/?"[^>]+>.?Shockwave Player 12.2.3.183' 'http://www.filehippo.com/download_shockwave/$1/'
    $page++
  }
  if (! $url1) {
