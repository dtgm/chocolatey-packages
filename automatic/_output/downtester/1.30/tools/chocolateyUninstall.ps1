...
if (Test-Path -PathType Container (Join-Path $chocoLib 'downtester.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package downtester, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\downtester.*).Name
  foreach ($i in $list) {
