...
if (Test-Path -PathType Container (Join-Path $chocoLib 'pstpassword.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package pstpassword, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\pstpassword.*).Name
  foreach ($i in $list) {
