...
if (Test-Path -PathType Container (Join-Path $chocoLib 'bulkrenameutility.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package bulkrenameutility, please also run the command:"
  $text += " `n`tchoco uninstall "
  $list = (Get-ChildItemDir $chocoLib\bulkrenameutility.*).Name
  foreach ($i in $list) {
    $text += "$i "
  }
  $text += "-y`n"
