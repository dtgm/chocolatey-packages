...
if (Test-Path -PathType Container (Join-Path $chocoLib 'sniffpass.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package sniffpass, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\sniffpass.*).Name
  foreach ($i in $list) {
