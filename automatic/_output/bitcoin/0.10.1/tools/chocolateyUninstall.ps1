...
if (Test-Path -PathType Container (Join-Path $chocoLib 'bitcoin.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package bitcoin, please also run the command:"
  Write-Host " `n`tchoco uninstall " -NoNewLine
  $list = (Get-ChildItemDir $chocoLib\bitcoin.*).Name
  foreach ($i in $list) {
