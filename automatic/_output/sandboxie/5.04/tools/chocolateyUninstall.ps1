...
if (Test-Path -PathType Container (Join-Path $chocoLib 'sandboxie.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package sandboxie, please also run the command:"
  Write-Host " `n`tchoco uninstall " -NoNewLine
  $list = (Get-ChildItemDir $chocoLib\sandboxie.*).Name
  foreach ($i in $list) {
