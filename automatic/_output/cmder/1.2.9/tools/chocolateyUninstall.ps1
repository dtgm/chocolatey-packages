...
if (Test-Path -PathType Container (Join-Path $chocoLib 'cmder.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package cmder, please also run the command:"
  Write-Host " `n`tchoco uninstall " -NoNewLine
  $list = (Get-ChildItemDir $chocoLib\cmder.*).Name
  foreach ($i in $list) {
