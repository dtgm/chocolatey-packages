...
if (Test-Path -PathType Container (Join-Path $chocoLib 'lili.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package lili, please also run the command:"
  Write-Host " `n`tchoco uninstall " -NoNewLine
  $list = (Get-ChildItemDir $chocoLib\lili.*).Name
  foreach ($i in $list) {
