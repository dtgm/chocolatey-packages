...
if (Test-Path -PathType Container (Join-Path $chocoLib 'mspass.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package mspass, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\mspass.*).Name
  foreach ($i in $list) {
