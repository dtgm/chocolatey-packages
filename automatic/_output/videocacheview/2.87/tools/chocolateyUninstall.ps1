...
if (Test-Path -PathType Container (Join-Path $chocoLib 'videocacheview.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package videocacheview, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\videocacheview.*).Name
  foreach ($i in $list) {
