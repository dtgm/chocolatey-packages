...
if (Test-Path -PathType Container (Join-Path $chocoLib 'keepass.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package keepass, please also run the command:"
  Write-Host " `n`tchoco uninstall " -NoNewLine
  $list = (Get-ChildItemDir $chocoLib\keepass.*).Name
  foreach ($i in $list) {
