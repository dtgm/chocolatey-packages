...
if (Test-Path -PathType Container (Join-Path $chocoLib 'serviwin.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package serviwin, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\serviwin.*).Name
  foreach ($i in $list) {
