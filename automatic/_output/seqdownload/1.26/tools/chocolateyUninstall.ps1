...
if (Test-Path -PathType Container (Join-Path $chocoLib 'seqdownload.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package seqdownload, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\seqdownload.*).Name
  foreach ($i in $list) {
