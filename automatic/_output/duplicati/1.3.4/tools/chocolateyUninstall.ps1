...
if (Test-Path -PathType Container (Join-Path $chocoLib 'duplicati.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package duplicati, please also run the command:"
  Write-Host " `n`tchoco uninstall " -NoNewLine
  $list = (Get-ChildItemDir $chocoLib\duplicati.*).Name
  foreach ($i in $list) {
