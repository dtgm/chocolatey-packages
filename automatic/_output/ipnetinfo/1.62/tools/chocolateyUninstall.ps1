...
if (Test-Path -PathType Container (Join-Path $chocoLib 'ipnetinfo.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package ipnetinfo, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\ipnetinfo.*).Name
  foreach ($i in $list) {
