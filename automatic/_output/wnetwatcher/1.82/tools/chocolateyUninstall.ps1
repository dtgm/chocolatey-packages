...
if (Test-Path -PathType Container (Join-Path $chocoLib 'wnetwatcher.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package wnetwatcher, please also run the command:"
  Write-Host " `n`tchoco uninstall " -NoNewLine
  $list = (Get-ChildItemDir $chocoLib\wnetwatcher.*).Name
  foreach ($i in $list) {
