...
if (Test-Path -PathType Container (Join-Path $chocoLib 'telegram.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package telegram, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\telegram.*).Name
  foreach ($i in $list) {
