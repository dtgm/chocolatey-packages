...
if (Test-Path -PathType Container (Join-Path $chocoLib 'winsshd.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package winsshd, please also run the command:"
  Write-Host " `n`tchoco uninstall " -NoNewLine
  $list = (Get-ChildItemDir $chocoLib\winsshd.*).Name
  foreach ($i in $list) {
