...
if (Test-Path -PathType Container (Join-Path $chocoLib 'electrum.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package electrum, please also run the command:"
  $text += " `n`tchoco uninstall "
  $list = (Get-ChildItemDir $chocoLib\electrum.*).Name
  foreach ($i in $list) {
    $text += "$i "
  }
  $text += "-y`n"
