...
if (Test-Path -PathType Container (Join-Path $chocoLib 'wul.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package wul, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\wul.*).Name
  foreach ($i in $list) {
