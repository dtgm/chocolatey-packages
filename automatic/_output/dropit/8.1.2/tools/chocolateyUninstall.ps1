...
if (Test-Path -PathType Container (Join-Path $chocoLib 'dropit.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package dropit, please also run the command:"
  Write-Host " `n`tchoco uninstall " -NoNewLine
  $list = (Get-ChildItemDir $chocoLib\dropit.*).Name
  foreach ($i in $list) {
