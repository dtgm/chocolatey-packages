...
if (Test-Path -PathType Container (Join-Path $chocoLib 'iepv.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package iepv, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\iepv.*).Name
  foreach ($i in $list) {
