...
if (Test-Path -PathType Container (Join-Path $chocoLib 'dispcalgui.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package dispcalgui, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\dispcalgui.*).Name
  foreach ($i in $list) {
