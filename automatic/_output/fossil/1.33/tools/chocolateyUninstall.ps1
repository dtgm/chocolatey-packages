...
if (Test-Path -PathType Container (Join-Path $chocoLib 'fossil.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package fossil, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\fossil.*).Name
  foreach ($i in $list) {
