...
if (Test-Path -PathType Container (Join-Path $chocoLib 'thebrain.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package thebrain, please also run the command:"
  Write-Host " `n`tchoco uninstall " -NoNewLine
  $list = (Get-ChildItemDir $chocoLib\thebrain.*).Name
  foreach ($i in $list) {
