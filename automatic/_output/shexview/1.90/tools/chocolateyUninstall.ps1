...
if (Test-Path -PathType Container (Join-Path $chocoLib 'shexview.*')) {
...
"@
  Write-Warning "To finish removing the program installed by package shexview, please also run the command:"
  Write-Host " `n`tcuninst " -NoNewLine
  $list = (Get-ChildItem -Directory $chocoLib\shexview.*).Name
  foreach ($i in $list) {
