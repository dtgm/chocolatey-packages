...
$mongoVersion = '{{PackageVersion}}'

$fileName = "mongodb-win32-i386-$mongoVersion"
$osBitness = Get-ProcessorBits
if ($osBitness -eq 64) {$fileName = "mongodb-win32-x86`_64-$mongoVersion"}
$url = "http://downloads.mongodb.org/win32/$fileName.zip"

$binRoot = "$env:systemdrive\"
### Using an environment variable to to define the bin root until we implement YAML configuration ###
if($env:ChocolateyBinRoot -ne $null){$binRoot = join-path $env:SystemDrive $env:ChocolateyBinRoot}
